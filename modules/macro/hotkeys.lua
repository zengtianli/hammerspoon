...
-- 存储已绑定的快捷键对象
local bound_hotkeys = {}
local macro_recorder = nil -- 延迟加载

-- 绑定宏快捷键
M.bind_macro_hotkeys = function()
    -- 延迟加载 macro_recorder 避免循环依赖
    if not macro_recorder then
        macro_recorder = require("modules.macro.recorder")
    end
    ...
    for i, config in ipairs(macro_hotkey_configs) do
        ...
        local hotkey = hs.hotkey.bind(modifiers, key, function()
            ...
            -- 动态获取函数
            local callback = macro_recorder[func_name]
            if callback and type(callback) == "function" then
                ...
                for k, v in pairs(macro_recorder or {}) do
...
-- 解绑宏快捷键
M.unbind_macro_hotkeys = function()
...
-- 重新绑定宏快捷键 (用于配置更新后)
M.rebind_macro_hotkeys = function()
    M.unbind_macro_hotkeys()
    M.bind_macro_hotkeys()
end

-- 获取宏快捷键配置信息
M.get_hotkey_info = function()
    local info = {}
    for i, config in ipairs(macro_hotkey_configs) do
...
-- 更新宏快捷键配置 (动态配置时使用)
M.update_hotkey_config = function(new_configs)
    macro_hotkey_configs = new_configs
    M.rebind_macro_hotkeys()
